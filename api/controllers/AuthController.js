...
module.exports = {
  ...
  login: (req, res) => {
    ...
    if (authConfig.wetland) {
      findUser = req.getRepository(User.Entity).findOne({[loginProperty]: params[loginProperty]}, {populate: true});
    } else {
      findUser = User['findOneBy' + _.upperFirst(loginProperty)](params[loginProperty]).populateAll();
    }
  ...
  signup: (req, res) => {
    ...
    if (authConfig.wetland) {
      ...
      findUser = User['findOneBy' + _.upperFirst(loginProperty)](params[loginProperty]);
    }
    ...
    sails.services.authservice.verifyToken(params.token)
      .then(decodedToken => {
        if (sails.config.auth.wetland) {
          ...
          return manager.getRepository(User.Entity).findOne(decodedToken.activate);
        }
